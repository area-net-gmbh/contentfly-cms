          ...
          function errorCallback(response) {
            ...
            if (response.status == 401) {
              ...
              var modalInstance = $uibModal.open({
                ...
                resolve: {
                  title: function () {
                    return response.data.type == 'Areanet\\Contentfly\\Classes\\Exceptions\\File\\FileExistsException' ? 'Datei überschreiben?' : 'Fehler beim Anlegen des Datensatzes';
                  },
                  body: function () {
                    return response.data.message;
                  },
              ...
              if (response.data.type == 'Areanet\\Contentfly\\Classes\\Exceptions\\File\\FileExistsException') {
                modalInstance.result.then(
